    ...
    def wessenplatten(self, n_layers, sigma, correlation_length):
        ...
        for i, n_layer in enumerate(n_layers[:-1]):
            ...
            roughness_envelope_l = (
                np.abs(x_lens_l) - x_lens_l.max()) / x_lens_l.max()
            roughness_envelope_r = (
                np.abs(x_lens_r) - x_lens_r.max()) / x_lens_r.max()

            h_lens_l = h_lens_l + roughness_envelope_l * sigma * (
                np.random.rand(len(x_lens_l)) - 0.5)
            h_lens_r = h_lens_r + roughness_envelope_r * sigma * (
                np.random.rand(len(x_lens_r)) - 0.5)

            array1 = np.array([x_lens_l, h_lens_l]).transpose()
            array2 = np.array([x_lens_r, h_lens_r]).transpose()
            t1.mask_from_array(
                r0=(0 * um, 0 * um),
                refraction_index=indice_average,
                array1=array1,
                array2=array2,
                x_sides=(-5 * mm, 5 * mm),
                angle=0 * degrees,
